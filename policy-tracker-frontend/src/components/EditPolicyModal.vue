                ...
                <q-card-section class="q-gutter-md scroll" style="max-height: 72vh;">
                    ...
                    <template v-if="form.type === 'SAGLIK'">
                        ...
                        <div class="text-subtitle2 text-weight-bold q-mb-xs">
                            ...
                            <!-- Birth Date (date-picker, Başlangıç/Bitiş Tarihi ile aynı format) -->
...
// Typed getter avoids unnecessary-type-assertion lint errors while
// preserving full type-safety in templates.
...
const trackedFields: (keyof PolicyForm)[] = [
    'customerId', 'type', 'premium', 'note', 'responsibleUserId', 'startDate', 'endDate', 'isActive', 'company'
];
...
const onModalShow = async () => {
    ...
    // ⚠️ form.value.type'ı set etmeden ÖNCE composable'daki type-change watch'ını
    // suprese ediyoruz. Aksi halde watch, type değişimini yakalayıp
    // typeSpecificFields'i getInitialTypeFields(...) ile sıfırlıyor ve az sonra
    // bizim fillTypeFieldsFromPolicy ile yazdığımız gerçek verinin üzerine geçiyordu
    // (Vue'nun 'pre'-flush watcher'ları senkron blok bitene kadar kuyrukta bekliyor).
    ...
    // Vue'nun scheduler kuyruğunun (suprese edilmiş) watch'ı flush etmesini bekle.
    // Böylece flag hâlâ true iken watch varsa çalışır ama hiçbir şey yapmaz;
    // fields'i asıl biz aşağıda dolduracağız.
    await nextTick();

    // Tip-özel alanları mevcut poliçe verisinden doldur
    ...
    // Watch'ı tekrar normale döndür (ör. modal ileride create/renew modunda
    // yeniden kullanılırsa type değişiklikleri yine doğru şekilde reset atsın)
